USE sql_hr;
DROP PROCEDURE IF EXISTS sp_refresh_manager_portfolio_summary;
DELIMITER $$
CREATE PROCEDURE sp_refresh_manager_portfolio_summary()
BEGIN
	...
	FROM(
		SELECT
			...
			ROUND(AVG(cs.total_outstanding_balance),2) AS avg_client_balance
